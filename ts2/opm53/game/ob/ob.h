...
typedef struct gfxchunk_s {
  short int texture;
  short int giftagoffset;
  short int vtxoffset;
  short int numvtx;
  short int ucodetype;
} gfxchunk;
...
void bgLightsKeepVisible(bglighttest **pList, float *Pos, int PosRoom,
                         int NumKeep);
...
obinst *obInstNew(struct prop_s *p, struct propdef_s *pd);
...
void obProcessGlows(struct prop_s *p, int partnum, partdef *part, mtx *pMat,
                    mtx *worldMtx);
...
int obInstLineTest(obinst *inst, float *StartPosOrig, float *DirOrig,
                   int infinite, float *hitpos, float *hitnorm);
void obInstSetOverrideStates(obinst *inst, int overridealphastate,
                             int overrideditherstate, int overridezbstate,
                             u8 fixedalpha);
void obInstSetOverrideCol(obinst *inst, float overrideambientscale, int colr,
                          int colg, int colb);
void obInstAllocateOverridePointers(obinst *inst);
void obInstNullOverridePointers(obinst *inst);
u32 *obInstGetOverrideChunk(obinst *inst, int partIndex, int gfxtype,
                            int overrideFlag, int *numchunkptr,
                            int *numgiftagsptr, int *numvtxptr);
void obInstAllocateOverrideChunk(obinst *inst, int partIndex, int gfxtype,
                                 int overrideFlags);
